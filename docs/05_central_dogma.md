...
Day-one **operational**: `tool_repair_attempts` (2) · `lease_ttl_ms` · budget defaults · `name_roster` · `honorific_set` (conferrals are immutable snapshots; roster edits cannot rewrite them — fails all three tests) · `bias_skew_threshold` (0.50) · `bias_pattern_window` (20) · `bias_pattern_threshold` (0.60) — the bias-legibility trio (Holy Standard §6.3) · `admission_batch_threshold` (50) · `admission_rate_window_ms` (3600000) · `admission_rate_threshold` (5) — the admission-legibility trio (SC-I07b; Book II §1): thresholds of notice on the Manifest, never of blocking — they change what the sovereign is told, not what any record means, and so sit operational under all three tests.
...
**Round 7 (2026-07-09, riding with Slice 10's delivery as its spec ordered):** the admission-legibility trio added to A.14's day-one operational list (ruling G11; SC-I07b) — `admission_batch_threshold`, `admission_rate_window_ms`, `admission_rate_threshold`, seeded by migration 0015 with the defaults recorded above. Graduated legibility for the threshold: past the constants the Manifest carries a standing notice with petition-style terminal answers; the gate never blocks on them and never goes silent.

**Round 8 (2026-07-10, riding with Slice 11b's delivery, author-sanctioned):** IV.4's closed list gains **retiring an environment** — the sole lever from LIVE/ORPHANED to DISSOLVED (A.8), a deliberate human retirement distinct from the dependency-loss ORPHANING a decommission cascades (doc 06:83). `retire_environment` takes a human actor and no job identity; the substrate refuses a job-uuid retirer as a `GATE_BYPASS_ATTEMPT`, exactly as the agent-author wall refuses one on the other reserved tables. The Slice 11b spec (`docs/dev/SLICE_11B.md` §0.2, §0.6) argues the semantics; migration 0019 raises the wall.
